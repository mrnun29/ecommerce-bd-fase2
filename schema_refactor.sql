...
-- Sistema Refactorizado con 3 Roles de Usuario
...
-- Tabla PROVEEDOR (información de empresas proveedoras)
...
-- Tabla PROVEEDOR_USUARIO (vincula usuarios tipo Proveedor con empresas)
CREATE TABLE IF NOT EXISTS `PROVEEDOR_USUARIO` (
  `id_proveedor` INT NOT NULL,
  `id_usuario` INT NOT NULL,
  PRIMARY KEY (`id_proveedor`, `id_usuario`),
  INDEX `fk_PROVEEDOR_USUARIO_USUARIO_idx` (`id_usuario` ASC),
...
-- Tabla PEDIDO (ahora relacionada directamente con USUARIO, no con CLIENTE)
CREATE TABLE IF NOT EXISTS `PEDIDO` (
  ...
  `id_usuario` INT NOT NULL,
  `procesado_por` INT NULL COMMENT 'ID del trabajador que procesó el pedido',
  PRIMARY KEY (`id_pedido`),
  INDEX `fk_PEDIDO_USUARIO_idx` (`id_usuario` ASC),
  INDEX `fk_PEDIDO_TRABAJADOR_idx` (`procesado_por` ASC),
  CONSTRAINT `fk_PEDIDO_USUARIO`
    FOREIGN KEY (`id_usuario`)
    REFERENCES `USUARIO` (`id_usuario`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `fk_PEDIDO_TRABAJADOR`
...
-- Tabla CARRITO (detalles de pedido)
...
CREATE TABLE IF NOT EXISTS `AVASTECE` (
  ...
  PRIMARY KEY (`PRODUCTO_id_producto`, `PROVEEDOR_id_proveedor`, `fecha_abastecimiento`),
